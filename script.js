document.addEventListener('DOMContentLoaded', function() {
  ...
  // Check if tree images need height adjustment for mobile view
  adjustTreeDetailsHeight();
  window.addEventListener('resize', adjustTreeDetailsHeight);
});

function adjustTreeDetailsHeight() {
  const treeDetailsWrapper = document.querySelector('.tree-details-wrapper');
  if (!treeDetailsWrapper) return;
  
  const activeImage = document.querySelector('.tree-image.active');
  if (activeImage) {
    if (window.innerWidth <= 768) {
      // For mobile, make sure the details wrapper is tall enough
      treeDetailsWrapper.style.minHeight = `${activeImage.offsetHeight}px`;
    } else {
      treeDetailsWrapper.style.minHeight = '400px';
    }
  }
}
...
function activateTree(treeType) {
  ...
  // Adjust height for mobile after changing trees
  setTimeout(adjustTreeDetailsHeight, 100);
}
...
function setupSwipeHandling() {
  const galleryElement = document.getElementById('galleryCurrentImage');
  let touchStartX = 0;
  let touchEndX = 0;
